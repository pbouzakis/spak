...
import UncaughtErrors from "./providers/UncaughtErrors";
...
export default class App {
    ...
    ready() {
        ...
        return this.events.publish.when("app.ready", readyEvent).then(() => {
    ...
    static runAsync(components, config, delegate) {
        return Q.try(() => new App(components, config, delegate))
            .then(() => _instance.bootstrap())
            .then(() => _instance.ready())
    ...
    localize(path, ...args) {
        return this._localization.localize(path, ...args);
    }
    ...
    dispatchAction(...args) {
        return this._actions.dispatch(...args)
            .catch((error) => this._uncaughtErrors.handleActionError(error));
    }
    ...
    static get UncaughtErrors() { return UncaughtErrors; }
}
