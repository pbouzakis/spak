...
function isNonYepModule(file, grunt) {
    var nonYepModules = grunt.config("yep").nonYepModulesRegex;
    ...
    return !grunt.file.doesPathContain("app", file) &&
           ...
           nonYepModules.test(relativeFile);
}

function shouldIgnore(file, grunt) {
    return isJadeTemplate(file) || isNonYepModule(file, grunt);
}
